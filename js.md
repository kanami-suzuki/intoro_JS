...
### DOM
- Document Object Model(DOM)とは、Webページを動的に書き換えるための仕組み
- JavaScriptで記述されたプログラムから、Webページにアクセスしてページを操作する
- DOMを操作するために必要となるのがDOMツリーの知識
- DOMツリー
  - HTMLドキュメント(HTMLファイル)をツリー構造として表現したもので、プログラムから利用するための集合体としての階層のことを指す。つまりノードを階層分けしたものがDOMツリーということになる
    - Document - html - head - meta
                      └ body...HTMLで記述される要素
- ノード
  - 文章を構成する要素、属性、テキストといったオブジェクトのこと。htmlタグやbodyタグなど各要素もノードと呼ぶDOMツリーの一つのオブジェクトがノード。ブラウザに表示されるブラウザを構成している各部品のことを「ブラウザオブジェクト」とも「Windowオブジェクト」とも呼ぶ
    - Windowオブジェクト：ブラウザを操作するための機能を集めたオブジェクト。ブラウザオブジェクトの階層構造の最上位に位置する
    - Documentオブジェクト：Windowオブジェクト内に表示されたHTMLで表現されているコンテンツを保持しているオブジェクト。Window内に表示されたドキュメントを操作するのはDocumentオブジェクトの役割
    - Locationオブジェクト：現在表示されているページのロケーションに関する情報を提供するオブジェクト。現在の表示URLやアドレスに関する情報を取得できる
    - Historyオブジェクト：ブラウザの履歴の操作。画面上に表示しているページの移動などの操作
    - Navigatorオブジェクト：ブラウザ名やバージョンなど、ブラウザ固有の情報を提供する
    - screenオブジェクト：ディスプレイに関する情報を提供する
    - formオブジェクト：Formに関する情報を提供する。Formの操作
    - Anchorオブジェクト：ページ上のアンカー(<a>)に関する情報を扱う
    - Imageオブジェクト：画像に関する情報の提供、画像を操作する機能の提供
    - Elementオブジェクト：HTMLドキュメントにおける、要素(タグ)のこと。h1やpタグなど

### windowオブジェクト
- ブラウザの画面に関係するプロパティやメソッドを束ねておく入れ物のようなもの
- グローバルオブジェクトのため、どこからでも呼び出せてすぐに使える
- console.log()やalert()などもwindowオブジェクト。windowは省略可能
  - window.console.log()
  - window.alert()

### イベント処理
- イベントとは
  - プログラムしているシステムの中で生じた動作や出来事のこと
    - ユーザーがボタンをクリックした
    - ユーザーが画面をスクロールした
    - ユーザーがキーボードのキーを押した
    - Webページのロードが完了した
...
- イベントハンドラとは
  - イベントに対して行う処理のこと
    - コンソールに'Hello'を出力する処理
- イベントリスナとは
  - イベントとイベントハンドラを紐づけているもの。イベントに対してイベントハンドラを登録する
    1. ボタンをクリックしたらという処理 イベント
    2. コンソールにHelloと出力 イベントハンドラ
    3. 1と2を紐づける イベントリスナ
- イベント駆動型プログラミング(イベントドリブン)
  - 何らかのイベントが発生したら、何らかのプログラムが動作するようにプログラミングする手法のこと